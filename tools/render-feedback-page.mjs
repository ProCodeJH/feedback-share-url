...
function renderMultiline(text, fallback) {
  const value = withFallback(text, fallback);
...
export function renderFeedbackPageHtml(raw = {}) {
  ...
  const coverIntro = `코딩쏙 학원입니다.<br><strong>${studentName} 학생</strong>의 오늘의 수업 피드백 안내드립니다.`;
...
<head>
  ...
  <style>
    :root {
      ...
      --shadow: 0 28px 90px rgba(20,43,33,0.16);
    }
    * { box-sizing: border-box; }
    ...
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at top left, rgba(185,135,69,0.22), transparent 28%),
        radial-gradient(circle at top right, rgba(34,74,58,0.18), transparent 32%),
        linear-gradient(180deg, #e8dfd0 0%, var(--paper) 44%, #ece4d8 100%);
    }
    ...
    .viewer-buttons { display: flex; gap: 8px; }
    button {
      appearance: none;
      border: 1px solid rgba(34,74,58,0.1);
      border-radius: 999px;
      padding: 13px 16px;
      font: inherit;
      font-weight: 800;
    ...
    .cover-card {
      ...
      border-radius: 32px;
      background: linear-gradient(145deg, rgba(20,43,33,0.98), rgba(34,74,58,0.94));
      color: white;
      text-align: center;
      box-shadow: 0 26px 70px rgba(20,43,33,0.22);
    }
    .cover-logo-wrap {
      display: flex;
      justify-content: center;
    ...
    .cover-kicker::before {
      ...
      height: 8px;
      border-radius: 50%;
      background: linear-gradient(135deg, #f0c27b, #c7954d);
    }
    .cover-title {
      margin: 22px 0 14px;
    ...
    .cover-message strong { color: #f2c98c; }
    .cover-sub {
      margin: 18px auto 0;
      max-width: 42ch;
    ...
    .page-copy { color: var(--ink-soft); font-size: 13px; line-height: 1.6; }
    .date-chip {
      padding: 11px 14px;
      border-radius: 18px;
      background: rgba(185,135,69,0.12);
      color: #6b4717;
      font-size: 12px;
      font-weight: 700;
    }
    ...
    .placeholder { color: rgba(89,112,99,0.62); font-style: italic; }
    .foot {
      margin-top: 20px;
      padding-top: 18px;
    ...
    @media (max-width: 900px) {
      ...
      .viewer-buttons { width: 100%; }
      .viewer-buttons button { flex: 1; }
    }
  </style>
</head>
<body>
  <div class="shell">
    <div class="viewer-nav">
      <div class="viewer-meta">
        ...
        <div id="page-caption">첫 페이지는 안내 표지이고, 다음 버튼을 누르면 실제 피드백이 열립니다.</div>
      </div>
    ...
    <article class="sheet">
        ...
        <section class="cover-page">
          <div class="cover-card">
            <div class="cover-logo-wrap">
              <img class="cover-logo" src="../logo.png" alt="코딩쏙 학원 로고">
            </div>
            <div class="cover-kicker">Class Feedback Notice</div>
            <h2 class="cover-title">학부모님<br>안녕하세요.</h2>
            <p class="cover-message">${coverIntro}</p>
      ...
      <section class="page" data-page="2">
        <div class="page-section-head">
          <div>
            <div class="page-title">Feedback Details</div>
            <div class="page-copy">실제 수업 피드백 내용을 이 페이지에서 이어서 확인합니다.</div>
          </div>
        ...
        <div class="mini-summary">
          <div>
            ...
            <div class="mini-name">${studentName}</div>
          </div>
          <div class="mini-text">첫 페이지에서 안내를 확인한 뒤, 이 페이지에서 실제 수업 피드백 내용을 이어서 봅니다.</div>
        </div>
        <div class="detail-top">
          ...
          <div class="focus-box">
            ...
            <p>${renderMultiline(data.learn, "오늘 배운 내용을 입력하지 않았습니다.")}</p>
          </div>
        </div>
        <section class="grid">
          <section class="card">
            ...
            <h3>세부 활동</h3>
            <ul class="list">${renderActivities(data.activities)}</ul>
          </section>
          <section class="card">
            <div class="index">02</div>
            <h3>아이의 반응 및 태도</h3>
            <p class="body-text">${renderMultiline(data.reaction, "아이의 반응 및 태도를 입력하지 않았습니다.")}</p>
          </section>
          <section class="card">
            <div class="index">03</div>
            <h3>이해도 및 진행 상황</h3>
            <p class="body-text">${renderMultiline(data.progress, "이해도 및 진행 상황을 입력하지 않았습니다.")}</p>
          </section>
          <section class="card">
            <div class="index">04</div>
            <h3>숙제</h3>
            <p class="body-text">${renderMultiline(data.homework, "숙제를 입력하지 않았습니다.")}</p>
          </section>
          <section class="card span-2">
            <div class="index">05</div>
            <h3>다음 수업 계획</h3>
            <p class="body-text">${renderMultiline(data.next, "다음 수업 계획을 입력하지 않았습니다.")}</p>
          </section>
          <section class="card span-2">
            <div class="index">06</div>
            <h3>추가 전달사항</h3>
            <p class="body-text">${renderMultiline(data.extra, "추가 전달사항을 입력하지 않았습니다.")}</p>
          </section>
        </section>
        <footer class="foot">
  ...
  <script>
    ...
    function updatePage(page) {
      ...
      pageCaption.textContent = currentPage === 1
        ? "첫 페이지는 안내 표지이고, 다음 버튼을 누르면 실제 피드백이 열립니다."
